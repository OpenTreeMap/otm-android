...
public class RequestGenerator {
    ...
    public void getPlotsNearLocation(double geoY, double geoX, ContainerRestHandler<PlotContainer> handler) {
        ...
        try {
            if (loginManager.isLoggedIn()) {
                client.getWithAuthentication(url, loginManager.loggedInUser.getUserName(),
                        loginManager.loggedInUser.getPassword(), null, handler);
            } else {
    ...
    public void getPlotsNearLocation(double geoY, double geoX, RequestParams rp,
        ...
        try {
            if (loginManager.isLoggedIn()) {
                client.getWithAuthentication(url, loginManager.loggedInUser.getUserName(),
                        loginManager.loggedInUser.getPassword(), rp, handler);
            } else {
     ...
     *
     * @param urlName
     *            Short URL slug name of instance
     */
    public void getInstanceInfo(String urlName, JsonHttpResponseHandler handler) {
    ...
    public void getPlotsNearLocation(double geoY, double geoX, boolean recent, boolean pending,
        ...
        try {
            if (loginManager.isLoggedIn()) {
                client.getWithAuthentication(url, loginManager.loggedInUser.getUserName(),
                        loginManager.loggedInUser.getPassword(), params, handler);
            } else {
    ...
    public void updatePlot(int id, Plot plot, AsyncHttpResponseHandler handler) throws UnsupportedEncodingException {
        if (loginManager.isLoggedIn()) {
            try {
                client.putWithAuthentication(getInstanceNameUri("plots/"), loginManager.loggedInUser.getUserName(),
    ...
    public void getUserEdits(Context context, User user, int offset, int count, AsyncHttpResponseHandler handler)
        ...
        if (user != null) {
            ...
            client.getWithAuthentication("/user/" + user.getId() + "/edits", loginManager.loggedInUser.getUserName(),
                    loginManager.loggedInUser.getPassword(), new RequestParams(params), handler);
        }
    }

    public void addUser(User user, AsyncHttpResponseHandler handler) throws JSONException, UnsupportedEncodingException {
        client.postWithAuthentication("/user/", loginManager.loggedInUser.getUserName(),
    ...
    public void deleteCurrentTreeOnPlot(Context context, int plotId, JsonHttpResponseHandler handler)
        ...
        client.deleteWithAuthentication(context, "/plots/" + plotId + "/tree", loginManager.loggedInUser.getUserName(),
                loginManager.loggedInUser.getPassword(), handler);
    }

    public void deletePlot(Context context, int plotId, JsonHttpResponseHandler handler) throws JSONException {

        client.deleteWithAuthentication(context, "/plots/" + plotId, loginManager.loggedInUser.getUserName(),
                loginManager.loggedInUser.getPassword(), handler);
    }

    public void addTreePhoto(Plot plot, Bitmap bm, JsonHttpResponseHandler handler) throws JSONException {
        String formattedPath = String.format("plots/%s/tree/photo", plot.getId());
        client.postWithAuthentication(getInstanceNameUri(formattedPath), bm, loginManager.loggedInUser.getUserName(),
                loginManager.loggedInUser.getPassword(), handler, PHOTOUPLOADTIMEOUT);
    }

    public void addProfilePhoto(Bitmap bm, JsonHttpResponseHandler handler) throws JSONException {
        User user = loginManager.loggedInUser;
        String formattedPath = String.format("user/%s/photo/profile", user.getId());
        client.postWithAuthentication(formattedPath, bm, user.getUserName(), user.getPassword(), handler,
                PHOTOUPLOADTIMEOUT);
    ...
    public void changePassword(Context context, String newPassword, JsonHttpResponseHandler handler)
        ...
        client.putWithAuthentication("/user/" + loginManager.loggedInUser.getId() + "/password",
                loginManager.loggedInUser.getUserName(), loginManager.loggedInUser.getPassword(), password, handler);
    }
    ...
    public void addTree(Plot plot, AsyncHttpResponseHandler handler) throws JSONException, UnsupportedEncodingException {

        client.postWithAuthentication(getInstanceNameUri("plots"), loginManager.loggedInUser.getUserName(),
                loginManager.loggedInUser.getPassword(), plot, handler);
    }

    public void rejectPendingEdit(int id, JsonHttpResponseHandler handler) throws UnsupportedEncodingException,
            JSONException {
    ...
    public void approvePendingEdit(int id, JsonHttpResponseHandler handler) throws UnsupportedEncodingException,
            JSONException {
